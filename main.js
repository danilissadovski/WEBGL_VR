...
function draw() {
  ...
  let matOrient= getRotationMatrix(sensor.x, sensor.y, sensor.z);
    // let p = document.getElementById("p")
    //         p.innerHTML=matOrient
  ...
  // }
  // else{
  gl.uniformMatrix4fv(shProgram.iModelViewMatrix, false, m4.multiply(matAccumMR, matOrient));
  // }
  // 
  ...
  // }
  // else{
    gl.uniformMatrix4fv(shProgram.iModelViewMatrix, false, m4.multiply(matAccumML, matOrient));
  // }
...
function CreateSurfaceData() {
  ...
  for (let u = 0; u <= uMax; u += uStep) {
    for (let v = 0; v <= vMax; v += vStep) {
      let vert = SnailSurface(u, v)
      let avert = SnailSurface(u + uStep, v)
      let bvert = SnailSurface(u, v + vStep)
      let cvert = SnailSurface(u + uStep, v + vStep)
